...
+ (DSActivityView *)activityViewForView:(UIView *)addToView withLabel:(NSString *)labelText width:(NSUInteger)labelWidth;
{
    ...
    return [[[self alloc] initForView:addToView withLabel:labelText width:labelWidth] autorelease];//TODO
}
...
CGFloat hScale = 0.5;//TODO

+ (void)setHScale:(CGFloat)scale; {
	hScale = scale;
}
...
- (void)layoutSubviews;
{
    ...
    borderFrame.origin.x = floor(0.5 * (self.frame.size.width - borderFrame.size.width));
    borderFrame.origin.y = floor(hScale * (self.frame.size.height - borderFrame.size.height));//TODO
    self.borderView.frame = borderFrame;
